...
class PhotoLikeView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        post = Post.objects.get(post_id=post_id)
        photo = Post_Picture.objects.filter(post=post, id=photo_id).first()
        likephoto = PhotoLike.objects.filter(photo=photo, user=user).first()
        if likephoto is not None:
            likephoto.delete()
            return Response({'like': False})
        else:
            new_photo_like = PhotoLike.objects.create(photo=photo, user=user)
            new_photo_like.save()
            return Response({"like": True})
